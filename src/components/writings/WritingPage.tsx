...
import {
  useActiveHeading,
  useMobileContentsVisibility,
} from './useWritingNavigation';
    ...
    <div className={styles.articlePage}>
      <header className={styles.articleHeader}>
        ...
        <div className={styles.articleMeta}>
          ...
          {writing.updatedAt && (
            <span>Updated {formatWritingDate(writing.updatedAt)}</span>
          )}
          {writing.series && (
            <span>
              Series: {writing.series.name} · Part {writing.series.order}
            </span>
          )}
        </div>
        {(writing.tags.length > 0 || writing.technologies.length > 0) && (
          <ul className={styles.chipList} aria-label="Writing metadata">
            {[...writing.tags, ...writing.technologies].map((item, index) => (
      ...
      <div
        className={`${styles.readingLayout} ${showToc ? '' : styles.withoutToc}`}
      >
        {showToc && (
          <nav ref={tocRef} className={styles.toc} aria-label="Contents">
            ...
            <ol>
              {headings.map((heading) => (
                <li
                  key={heading.id}
                  className={heading.depth === 3 ? styles.tocNested : undefined}
                >
                  <a
                    href={`#${heading.id}`}
                    aria-current={
...
export default function WritingPage() {
  ...
  usePageTitle(writing?.title ?? 'Not Found');
