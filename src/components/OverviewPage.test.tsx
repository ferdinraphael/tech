...
describe('overview interactions', () => {
  ...
  it('starts neutral while showing Little Worlds as featured desktop content', () => {
    renderOverview();
    expect(
      screen.getByText('No constellation node selected.'),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('article', { name: /Little Worlds featured content/ }),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText('Featured, not selected'),
  ...
  it('selects Projects, switches context, and clears with Escape', async () => {
    ...
    expect(
      screen.getByRole('article', { name: /Projects selected content/ }),
    ).toBeInTheDocument();
  ...
  it('renders approved Little Worlds selected content', async () => {
    ...
    expect(
      within(panel).getByRole('link', { name: /Live Demo/ }),
    ).toHaveAttribute('href', links.littleWorldsDemo);
  ...
  it('opens Built & Published as a top-level constellation category', async () => {
    ...
    renderOverview();
    await user.click(
      screen.getByRole('button', { name: /^Built & Published\./ }),
    );
    const panel = screen.getByRole('article', {
      name: /Built & Published selected content/,
    });
    expect(
      within(panel).getByRole('link', { name: /View Built & Published/ }),
  ...
  it('selects Wildpath without replacing Little Worlds as the featured project', async () => {
    ...
    const panel = within(
      screen.getByRole('article', { name: 'Wildpath selected content' }),
    );
    expect(panel.getByText('Game Development')).toBeInTheDocument();
    expect(panel.getByText('Public project')).toBeInTheDocument();
    expect(panel.getByRole('link', { name: 'Play Wildpath' })).toHaveAttribute(
      'href',
      links.wildpathDemo,
    );
    expect(
      panel.queryByRole('link', { name: 'Repository' }),
    ).not.toBeInTheDocument();
    const featured = within(
      screen.getByRole('region', { name: 'Featured Project' }),
    );
    expect(
      featured.getByRole('heading', { name: 'Little Worlds' }),
    ).toBeInTheDocument();
    expect(
      featured.queryByRole('heading', { name: 'Wildpath' }),
    ).not.toBeInTheDocument();
    await user.keyboard('{Escape}');
    expect(
      screen.getByRole('article', { name: 'Little Worlds featured content' }),
  ...
  it('renders mobile context inline, preserves relationships, updates, and clears', async () => {
    ...
    const inline = screen.getByRole('region', {
      name: /Little Worlds inline details/,
    });
    expect(
      within(inline).getByRole('article', {
        name: /Little Worlds selected content/,
      }),
    ).toBeInTheDocument();
    ...
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: /Close Little Worlds details/ }),
    ).not.toBeInTheDocument();
    expect(
      screen
    ...
    const updatedInline = screen.getByRole('region', {
      name: /Projects inline details/,
    });
    expect(
      within(updatedInline).getByRole('heading', { name: 'Projects' }),
    ).toBeInTheDocument();
    expect(projects).toHaveAttribute('aria-pressed', 'true');
    expect(trigger).toHaveAttribute('aria-pressed', 'false');

    await user.click(
      within(updatedInline).getByRole('button', { name: 'Clear selection' }),
    );
    expect(
      screen.queryByRole('region', { name: /inline details/ }),
    ).not.toBeInTheDocument();
  ...
  it('uses immediate scrolling for reduced-motion mobile selection', async () => {
    ...
    vi.mocked(window.matchMedia).mockImplementation((query: string) => ({
      matches:
        query.includes('767px') || query.includes('prefers-reduced-motion'),
      media: query,
      onchange: null,
      addListener: vi.fn(),
      removeListener: vi.fn(),
  ...
  it('derives the latest published writing and never previews a draft', async () => {
    renderOverview();
    const latestTitle = await screen.findByRole('heading', {
    ...
    expect(
      within(latest).getByText(/Published Jun 21, 2026/),
    ).toBeInTheDocument();
    expect(
      within(latest).getByRole('link', { name: /Read article/ }),
    ).toHaveAttribute(
      'href',
    ...
    expect(
      screen.queryByText(/Technical writing framework preview/i),
    ).not.toBeInTheDocument();
  });
  ...
  it('previews books, tools, and differentiated service metadata from shared data', () => {
    ...
    expect(
      within(books).getByRole('heading', { name: /C# Debugging Drills/ }),
    ).toBeInTheDocument();
    expect(
      within(books).getByRole('heading', {
        name: /SQL Data Cleaning Cookbook/,
      }),
    ).toBeInTheDocument();
    expect(within(books).getAllByRole('article')).toHaveLength(2);
    expect(
      within(books).getAllByRole('link', { name: /View on Amazon/ }),
    ).toHaveLength(2);

    const tools = screen.getByRole('region', { name: 'Recent Tools' });
    expect(
      within(tools).getByText(/local-first .env comparison tool/i),
    ).toBeInTheDocument();
    expect(
      within(tools).getByText(/offline browser tool for generating RPG/i),
    ).toBeInTheDocument();
    expect(
      within(tools).getByRole('link', { name: /View EnvGuard/ }),
    ).toHaveAttribute('href', 'https://payhip.com/b/KJzvD');
    expect(
      within(tools).getByRole('link', { name: /View on itch.io/ }),
    ).toHaveAttribute('href', 'https://ferdinraphael.itch.io/rpg-data-forge');
    expect(
      within(tools).queryByRole('link', { name: /Pro version/ }),
    ).not.toBeInTheDocument();
    ...
    for (const [title, slug] of [
      ...
      expect(
        within(services).getByRole('link', { name: `Explore ${title}` }),
      ).toHaveAttribute('href', `/services/${slug}/`);
    }
    expect(
      within(services).getByText('All services are remote.'),
    ).toBeInTheDocument();
