...
describe('foundation routes', () => {
  it('keeps the trailing-slash deployment base on root links without duplicating it on nested links', () => {
    render(
      <MemoryRouter basename="/tech/" initialEntries={['/tech/services/']}>
        <App />
      </MemoryRouter>,
    );
    const nav = within(
    ...
    expect(nav.getByRole('link', { name: 'Services' })).toHaveAttribute(
      'href',
      '/tech/services/',
    );
    expect(nav.getByRole('link', { name: 'Services' })).toHaveAttribute(
    ...
    expect(nav.getByRole('link', { name: 'Projects' })).toHaveAttribute(
      'href',
      '/tech/projects/',
    );
  });
  ...
  it.each([
    ...
    '/services/software-development/',
  ])('keeps active navigation and canonical hrefs at %s', (path) => {
    renderRoute(path);
    const nav = within(
      screen.getByRole('navigation', { name: 'Primary navigation' }),
    );
    ...
    expect(nav.getByRole('link', { name: 'Projects' })).not.toHaveAttribute(
      'aria-current',
    );
    expect(nav.getByRole('link', { name: 'Projects' })).toHaveAttribute(
      'href',
  ...
  it.each([
    [
      '/projects',
      'Public software projects including Little Worlds and Wildpath.',
    ],
    [
      '/services',
      'Remote software development, technical consulting, and mentoring services.',
    ],
    [
      '/services/mentoring-teaching',
      'Remote 1-on-1 technical learning, developer mentoring, and small-group training.',
    ],
  ])(
    'sets public description, social metadata and one canonical for %s',
    (path, description) => {
      ...
      ).toHaveAttribute('content', document.title);
      expect(
      ...
      ).toHaveAttribute('href', `https://ferdinraphael.github.io/tech${path}/`);
      expect(
        document.head.querySelector('meta[property="og:url"]'),
      ).toHaveAttribute(
        'content',
        `https://ferdinraphael.github.io/tech${path}/`,
      );
      expect(
  ...
  it('clears unknown-route canonicals and restores indexable metadata on navigation', () => {
    ...
    expect(
      document.head.querySelector('link[rel="canonical"]'),
    ).toHaveAttribute('href', 'https://ferdinraphael.github.io/tech/');
    expect(
      document.head.querySelectorAll('link[rel="canonical"]'),
    ).toHaveLength(1);
    expect(document.head.querySelector('meta[name="robots"]')).toHaveAttribute(
  ...
  it('uses canonical target metadata after a legacy redirect', async () => {
    ...
    await waitFor(() =>
      expect(
        ...
        'href',
        'https://ferdinraphael.github.io/tech/writings/',
      ),
    );
    expect(
      document.head.querySelectorAll('link[rel="canonical"]'),
  ...
  it('renders the published writing separately from development drafts', async () => {
    ...
    expect(
      await screen.findByRole(
        'heading',
        { level: 1, name: /Writing about software, systems/i },
        { timeout: 5_000 },
      ),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole('heading', { name: 'No published writings yet.' }),
    ...
    expect(
      within(publishedCard!).getByText('Published Jun 21, 2026'),
    ).toBeInTheDocument();
    expect(within(publishedCard!).queryByText('DRAFT')).not.toBeInTheDocument();
    expect(
      within(publishedCard!).getByRole('link', { name: 'Read writing' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { name: 'Draft previews' }),
    ).toBeInTheDocument();
    const frameworkPreviewCard = screen
      .getByRole('heading', { name: 'Technical writing framework preview' })
      .closest('article');
    expect(frameworkPreviewCard).not.toBeNull();
    expect(
      within(frameworkPreviewCard!).getByText('Connected to Little Worlds'),
    ).toBeInTheDocument();
    expect(
      within(frameworkPreviewCard!).getByText('DRAFT'),
    ).toBeInTheDocument();
    expect(
      within(frameworkPreviewCard!).getByText('Article'),
    ).toBeInTheDocument();
    expect(
      within(frameworkPreviewCard!).getByRole('link', {
        name: 'Preview draft',
      }),
    ).toBeInTheDocument();
  ...
  it('keeps the public navigation focused on the launch information architecture', () => {
    ...
    expect(
      within(navigation).queryByRole('link', { name: 'Profile' }),
    ).not.toBeInTheDocument();
  });
  ...
  it('keeps the mobile drawer limited to public navigation, GitHub, and Email', () => {
    ...
    expect(
      within(drawer).queryByText('Existing identity site'),
    ).not.toBeInTheDocument();
    expect(
      within(drawer).queryByRole('link', { name: 'Profile' }),
    ).not.toBeInTheDocument();
  });
  ...
  it('renders verified Built & Published shelves with their exact actions', () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Tool Shelf' }),
    ).toBeInTheDocument();
    const csharpBook = screen
      .getByRole('heading', { name: /C# Debugging Drills/ })
      .closest('article')!;
    const sqlBook = screen
      .getByRole('heading', { name: /SQL Data Cleaning Cookbook/ })
      .closest('article')!;
    const envGuard = screen
      .getByRole('heading', { name: 'EnvGuard' })
      .closest('article')!;
    const rpgDataForge = screen
      .getByRole('heading', { name: 'RPG Data Forge' })
      .closest('article')!;
    expect(
      within(csharpBook).getByRole('link', { name: /View on Amazon/ }),
    ).toHaveAttribute(
      'href',
      'https://www.amazon.com/Debugging-Drills-Real-World-Bugs-Find-ebook/dp/B0HF8MLZ52/',
    );
    expect(
      within(sqlBook).getByRole('link', { name: /View on Amazon/ }),
    ).toHaveAttribute(
      'href',
      'https://www.amazon.in/SQL-Data-Cleaning-Cookbook-Real-World-ebook/dp/B0HF8KL378',
    );
    expect(
      within(envGuard).getByRole('link', { name: /Free version/ }),
    ).toHaveAttribute('href', 'https://payhip.com/b/KJzvD');
    expect(
      within(envGuard).getByRole('link', { name: /Pro version/ }),
    ).toHaveAttribute('href', 'https://payhip.com/b/r3Tn7');
    expect(
      within(rpgDataForge).getByRole('link', { name: /View on itch.io/ }),
  ...
  it('makes the focused project scope and Built & Published relationship explicit', () => {
    ...
    expect(
      screen.getByText('Deterministic simulation worlds'),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Evolving microbes with observable behaviour/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Interactive browser-based simulation/),
    ).toBeInTheDocument();
    const outputRelationship = screen.getByText(
      /Smaller finished tools and publications/,
    );
    expect(
      within(outputRelationship).getByRole('link', {
        name: 'Built & Published',
      }),
    ).toHaveAttribute('href', '/built-and-published/');
  });

  it('renders both public projects with only their available external actions', () => {
    renderRoute('/projects');
    const littleWorlds = within(
      screen.getByRole('region', { name: 'Little Worlds' }),
    );
    const wildpath = within(screen.getByRole('region', { name: 'Wildpath' }));
    expect(
      littleWorlds.getByRole('link', { name: 'Live Demo' }),
    ).toHaveAttribute('href', 'https://ferdinraphael.github.io/little-worlds');
    expect(
      littleWorlds.getByRole('link', { name: 'Repository' }),
    ).toHaveAttribute(
      'href',
      'https://github.com/ferdinraphael/little-worlds/',
    );
    const play = wildpath.getByRole('link', { name: 'Play Wildpath' });
    expect(play).toHaveAttribute(
      'href',
      'https://ferdinraphael.github.io/wildpath/',
    );
    ...
    expect(
      wildpath.queryByRole('link', { name: 'Repository' }),
    ).not.toBeInTheDocument();
    expect(
      wildpath.getByText('Responsive browser gameplay with touch support'),
    ).toBeInTheDocument();
  });
  ...
  it('loads the published writing without draft metadata', async () => {
    ...
    expect(
      await screen.findByRole(
        'heading',
        { level: 1, name: 'When the Workaround Becomes the Architecture' },
        { timeout: 5_000 },
      ),
    ).toBeInTheDocument();
    expect(document.title).toBe(
      'When the Workaround Becomes the Architecture — Ferdin Raphael',
    );
    expect(
      document.head.querySelector('meta[name="description"]'),
    ).toHaveAttribute(
      'content',
      writingCatalogue.getBySlug(
    ...
    expect(
      ...
      'href',
      'https://ferdinraphael.github.io/tech/writings/when-the-workaround-becomes-the-architecture/',
    );
    expect(screen.getByText('ARTICLE')).toBeInTheDocument();
    expect(screen.getByText('Published Jun 21, 2026')).toBeInTheDocument();
  ...
  it('loads a draft writing directly in development with Writings navigation active', async () => {
    ...
    expect(
      await screen.findByRole(
        'heading',
        { level: 1, name: 'Technical writing framework preview' },
        { timeout: 5_000 },
      ),
    ).toBeInTheDocument();
    expect(document.title).toBe(
      'Technical writing framework preview — Ferdin Raphael',
    );
    expect(document.head.querySelector('link[rel="canonical"]')).toBeNull();
    expect(document.head.querySelector('meta[name="robots"]')).toHaveAttribute(
      'content',
    ...
    expect(
      screen.getByRole('heading', { name: 'Little Worlds' }),
    ).toBeInTheDocument();
    expect(
      screen
        .getAllByRole('link', { name: 'Writings' })
  ...
  it('renders an intentional writing-not-found state for an unknown canonical slug', async () => {
    ...
    expect(
      await screen.findByRole('heading', {
        name: 'That writing is not available.',
      }),
    ).toBeInTheDocument();
    expect(document.title).toBe('Not Found — Ferdin Raphael');
    expect(
      screen.getByRole('link', { name: /Return to Writings/ }),
  ...
  it.each([
    ['/notes', '/writings/'],
    [
      '/notes/when-the-workaround-becomes-the-architecture',
      '/writings/when-the-workaround-becomes-the-architecture/',
    ],
    [
      '/notes/framework-preview?mode=review#equivalent-examples',
      '/writings/framework-preview?mode=review#equivalent-examples',
    ],
  ])(
    'redirects legacy %s with replace-style canonical location',
  ...
  it('renders a useful invalid route', () => {
    renderRoute('/not-a-real-place');
    expect(
      screen.getByRole('heading', { name: /not in the constellation/ }),
    ).toBeInTheDocument();
    expect(
