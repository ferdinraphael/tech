...
describe('repository technical-writing content', () => {
  ...
  it('keeps the framework preview draft-only with article format metadata', () => {
    const preview = writingCatalogue.all.find(
      ({ slug }) => slug === 'framework-preview',
    );
    expect(preview).toEqual(
      expect.objectContaining({ draft: true, format: 'article' }),
    );
  ...
  it('keeps the language-aware preview development-only with validated variants', () => {
    ...
    expect(
      preview?.segments.filter(({ type }) => type === 'language-content'),
    ).toHaveLength(5);
    expect(
      preview?.segments.filter(({ type }) => type === 'code-tabs'),
    ).toHaveLength(2);
    expect(
      preview?.segments.filter(({ type }) => type === 'runtime-model'),
    ).toHaveLength(7);
    expect(
      preview?.headings.filter(
        ({ text }) => text === 'Copy the object reference',
      ),
    ).toHaveLength(1);
    expect(
      preview?.headings.filter(({ text }) => text === 'Reassign one variable'),
    ).toHaveLength(1);
    expect(
      preview?.headings.filter(({ text }) => text === 'Change a scalar value'),
    ).toHaveLength(1);
    expect(
      preview?.headings.filter(({ text }) => text === 'Copy a scalar value'),
    ).toHaveLength(1);
    expect(preview?.headings.some(({ text }) => text === 'Current')).toBe(
      false,
    );
    expect(
      preview?.headings.some(({ text }) => text === 'Before reassignment'),
    ).toBe(false);
    expect(
      preview?.headings.some(({ text }) => text === 'Before assignment'),
    ).toBe(false);
    expect(
      preview?.headings.some(({ text }) => text === 'Before rebinding'),
    ).toBe(false);
    expect(writingCatalogue.published).not.toContain(preview);
  });
