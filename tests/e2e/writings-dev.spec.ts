...
test.skip(
  process.env.WRITINGS_DEV_PREVIEW !== '1',
  'Runs only against the bounded development preview.',
);

const writingPath = './writings/framework-preview';
const languageAwareWritingPath = './writings/language-aware-preview';

test('language-aware prose and code stay synchronized in single and Compare reading', async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole('heading', {
      level: 1,
      name: 'Language-aware writing preview',
    }),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole('tab', { name: 'C#', selected: true }),
  ).toHaveCount(2);
  await expect(page.getByRole('tabpanel').nth(0)).toContainText(
    'int count = 10;',
  );
  ...
  await expect(
    page.getByText('Variable count directly contains the int value 10.'),
  ).toBeAttached();
  await expect(
    page.getByText(
  ...
  await expect(
    csharpModels.nth(2).locator('[data-runtime-entity="variable"]'),
  ).toHaveCount(2);
  await expect(
    csharpModels.nth(2).locator('[data-runtime-entity="object"]'),
  ).toHaveCount(1);
  ...
  await expect(
    csharpModels.nth(3).locator('[data-runtime-changed="true"]'),
  ).toHaveCount(1);
  await expect(
    csharpModels.nth(4).locator('[data-runtime-topology="split-target"]'),
  ).toHaveCount(1);
  await expect(
    csharpModels.nth(4).locator('[data-runtime-relationship-changed="true"]'),
  ).toHaveCount(1);
  await expect(
    csharpModels.nth(5).locator('[data-runtime-changed="true"]'),
  ).toHaveCount(1);
  await expect(
    csharpModels.nth(6).locator('[data-runtime-topology="direct-values"]'),
  ).toHaveCount(2);
  ...
  await page.screenshot({
    path: 'visual-review/1536-language-aware-default.png',
    fullPage: false,
  });
  ...
  await expect(
    page.getByRole('tab', { name: 'Java', selected: true }),
  ).toHaveCount(2);
  await expect(page.getByRole('tabpanel').nth(0)).toContainText(
    'int count = 10;',
  );
  ...
  await expect(
    page.getByRole('region', { name: 'Java runtime model' }),
  ).toHaveCount(7);
  await expect(
    page.getByText(
  ...
  await expect(
    page
      .getByRole('region', { name: 'Java runtime model' })
      .nth(4)
      .getByText(
        /After the reassignment, a still refers to the original Counter object/,
      ),
  ).toBeAttached();
  await expect(
    page
  ...
  expect(
    Math.abs((await page.evaluate(() => window.scrollY)) - scrollBeforeJava),
  ).toBeLessThanOrEqual(2);
  expect(
    await page.evaluate(() =>
      window.localStorage.getItem('ferdinraphael.tech.preferred-code-language'),
    ),
  ).toBe('java');
  ...
  await expect(
    page.getByRole('tab', { name: 'Python', selected: true }),
  ).toHaveCount(2);
  ...
  await expect(
    page.getByRole('region', { name: 'Python runtime model' }),
  ).toHaveCount(7);
  await expect(
    page.getByText(
  ...
  await expect(
    page.getByText(
      /After the mutation, a and b are still bound to the same Counter object/,
    ),
  ).toBeAttached();
  await expect(
    page.getByText(
      /After the rebinding, a remains bound to the original Counter object/,
    ),
  ).toBeAttached();
  await expect(
    page
      .getByRole('region', { name: 'Python runtime model' })
      .nth(5)
  ...
  await page.evaluate(() => {
    const copied: string[] = [];
    (
      window as typeof window & { __copiedComparedCode?: string[] }
    ).__copiedComparedCode = copied;
  ...
  const codeComparisons = page.getByRole('region', {
    name: 'Equivalent code comparison',
  });
  await expect(codeComparisons).toHaveCount(2);
  await expect(codeComparisons.first().locator('p')).toHaveText([
    'C#',
    'Java',
    'Python',
  ]);
  await expect(
    codeComparisons.first().locator('code.language-csharp .hljs-number'),
  ).toHaveText('10');
  await expect(
    codeComparisons.first().locator('code.language-java .hljs-number'),
  ).toHaveText('10');
  await expect(
    codeComparisons.first().locator('code.language-python .hljs-number'),
  ).toHaveText('10');
  ...
  for (const [index, [language, expected]] of expectedComparedCode.entries()) {
    await codeComparisons
      .nth(Math.floor(index / 3))
      .getByRole('button', { name: `Copy ${language} code` })
      .click();
    const copied = await page.evaluate(
      () =>
        (window as typeof window & { __copiedComparedCode?: string[] })
          .__copiedComparedCode,
    );
  ...
  await expect(
    comparisons
      .first()
      .locator('p')
      .filter({ hasText: /^(C#|Java|Python)$/ }),
  ).toHaveText(['C#', 'Java', 'Python']);
  await expect(page.getByText(/whose value is the integer/)).toBeVisible();
  await expect(page.getByText(/primitive type/)).toBeVisible();
  await expect(page.getByText(/bound to an integer object/)).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole('region', { name: 'C# runtime model' }),
  ).toHaveCount(0);
  await expect(
    page.getByRole('region', { name: 'Java runtime model' }),
  ).toHaveCount(0);
  ...
  await expect(
    retainedSplit.locator('[data-runtime-object-identity="new"]'),
  ).toHaveCount(1);
  const retainedScalarCopy = page
    .getByRole('region', { name: 'Python runtime model' })
    .nth(6);
  ...
  await page.screenshot({
    path: 'visual-review/1536-language-aware-compare.png',
    fullPage: false,
  });
  ...
  for (const viewport of [
    ...
    await expect(page.getByRole('radio', { name: 'Compare' })).toBeChecked();
    await expect(
      page.getByRole('region', { name: 'Language comparison' }),
    ).toHaveCount(5);
    await expect(
      page.getByRole('region', { name: 'Equivalent code comparison' }),
    ).toHaveCount(2);
    await expect(
      page.getByRole('region', { name: 'Python runtime model' }),
    ).toHaveCount(7);
  ...
  await expect(page.getByRole('region', { name: /runtime model/ })).toHaveCount(
    0,
  );
  await expect(
    page.getByRole('tab', { name: 'TypeScript' }).first(),
  ).toBeVisible();
});

test('runtime models including mutation and reassignment remain readable and bounded', async ({
  page,
}) => {
  for (const viewport of [
    ...
    await expect(
      models.first().getByText('count', { exact: true }),
    ).toBeVisible();
    await expect(
      models.first().getByText('int', { exact: true }).last(),
    ).toBeVisible();
    await expect(
      models.first().getByText('10', { exact: true }).last(),
    ).toBeVisible();
    await expect(
      models.nth(1).getByText('Counter', { exact: true }).last(),
    ).toBeVisible();
    ...
    await expect(
      sharedModel.locator('[data-runtime-entity="variable"]'),
    ).toHaveCount(2);
    await expect(
      sharedModel.locator('[data-runtime-entity="object"]'),
    ).toHaveCount(1);
    await expect(
      sharedModel.locator('[data-runtime-entity="variable"]').nth(0),
    ).toContainText('a');
    await expect(
      sharedModel.locator('[data-runtime-entity="variable"]').nth(1),
    ).toContainText('b');
    ...
    expect(
      await sharedModel
        .getByLabel('C# code', { exact: true })
        .evaluate((element) => getComputedStyle(element).overflowX),
    ).toBe('auto');
    if (viewport.suffix) {
      const csharpCapture =
        viewport.width === 375
          ? sharedModel.locator('figure').last()
          : sharedModel;
    ...
    await expect(
      mutationModel.locator('[data-runtime-entity="object"]'),
    ).toHaveCount(2);
    await expect(
      mutationModel.locator('[data-runtime-entity="variable"]'),
    ).toHaveCount(4);
    await expect(
      mutationModel.locator('[data-runtime-changed="true"]'),
    ).toHaveCount(1);
    await expect(mutationModel.getByText('object mutated')).toBeVisible();
    await expect(
      mutationModel.getByText(/still refer to the same Counter object/),
    ).toBeAttached();
    if (viewport.suffix) {
    ...
    const splitAfter = splitModel.locator(
      '[data-runtime-topology="split-target"]',
    );
    await expect(
      splitModel.locator('[data-runtime-entity="object"]'),
    ).toHaveCount(3);
    await expect(
      splitAfter.locator('[data-runtime-object-identity="original"]'),
    ).toHaveCount(1);
    await expect(
      splitAfter.locator('[data-runtime-object-identity="new"]'),
    ).toHaveCount(1);
    await expect(
      splitAfter.locator('[data-runtime-relationship-changed="true"]'),
    ).toHaveCount(1);
    ...
    await expect(
      splitModel.getByText(/a still refers to the original Counter object/),
    ).toBeAttached();
    if (viewport.suffix) {
    ...
    await expect(
      scalarChange.locator('[data-runtime-changed="true"]'),
    ).toHaveCount(1);
    await expect(
      scalarChange.getByText(
        /same variable directly contains the int value 20/,
      ),
    ).toBeAttached();
    const scalarCopy = models.nth(6);
    await expect(
      scalarCopy.locator('[data-runtime-topology="direct-values"]'),
    ).toHaveCount(2);
    await expect(
      scalarCopy.locator('[data-runtime-changed="true"]'),
    ).toHaveCount(1);
    await expect(
      scalarCopy.getByText(/a still contains 10 while b directly contains 20/),
    ).toBeAttached();
    if (viewport.suffix) {
    ...
    await page.getByRole('radio', { name: 'Python' }).click();
    const pythonShared = page
      .getByRole('region', { name: 'Python runtime model' })
      .nth(2);
    await expect(
      pythonShared.locator('[data-runtime-entity="name"]'),
    ).toHaveCount(2);
    await expect(
      pythonShared.locator('[data-runtime-entity="object"]'),
    ).toHaveCount(1);
    ...
    if (viewport.suffix) {
      const pythonCapture =
        viewport.width === 375
          ? pythonShared.locator('figure').last()
          : pythonShared;
      ...
      const pythonSplit = page
        .getByRole('region', { name: 'Python runtime model' })
        .nth(4);
      await expect(
        pythonSplit.locator('[data-runtime-entity="name"]'),
      ).toHaveCount(4);
      await expect(
        pythonSplit.getByText(/a remains bound to the original Counter object/),
      ).toBeAttached();
      await pythonSplit.screenshot({
        path: `visual-review/runtime-model-rebinding-python-${viewport.suffix}.png`,
      });
      const pythonScalarCopy = page
        .getByRole('region', { name: 'Python runtime model' })
        .nth(6);
      await expect(
        pythonScalarCopy.locator('[data-runtime-object-identity="original"]'),
      ).toHaveCount(2);
      await expect(
        pythonScalarCopy.locator('[data-runtime-object-identity="new"]'),
      ).toHaveCount(1);
...
test('desktop writing tracks active headings, direct hashes, history, tabs, copy, and review states', async ({
  page,
  context,
}) => {
  ...
  await expect(
    page.getByRole('heading', {
      level: 1,
      name: 'Technical writing framework preview',
    }),
  ).toBeVisible();
  ...
  await expect(
    contents.getByRole('link', { name: 'What this fixture checks' }),
  ).toHaveAttribute('aria-current', 'location');
  await expect(
    page.getByRole('button', { name: /Return to this writing's Contents/ }),
  ).toHaveCount(0);
  await expect(
    page.getByRole('heading', { name: 'Related projects' }),
  ).toBeAttached();
  await page.screenshot({
    path: 'visual-review/1536-writings-framework-top.png',
    fullPage: false,
  });

  const examplesLink = contents.getByRole('link', {
    name: 'Equivalent examples',
    exact: true,
  });
  ...
  const examplesHeading = page.getByRole('heading', {
    name: 'Equivalent examples',
  });
  ...
  await page.screenshot({
    path: 'visual-review/1536-writings-equivalent-examples-active.png',
    fullPage: false,
  });

  const nestedLink = contents.getByRole('link', {
    name: 'Graceful preference fallback',
  });
  ...
  const finalLink = contents.getByRole('link', {
    name: 'Portable Markdown elements',
  });
  ...
  await page.screenshot({
    path: 'visual-review/1536-writings-portable-markdown-active.png',
    fullPage: false,
  });

  await contents
    .getByRole('link', { name: 'What this fixture checks' })
    .focus();
  await expect(
    contents.getByRole('link', { name: 'What this fixture checks' }),
  ).toBeFocused();
  ...
  await expect(
    visiblePanel.getByRole('button', { name: 'Copy Python code' }),
  ...
  expect(
    directHeadingBox && directHeadingBox.y >= 66 && directHeadingBox.y < 864,
  ).toBeTruthy();

  await examplesHeading.evaluate((heading) =>
    window.scrollTo(0, heading.offsetTop + 350),
  );
  await page
    .getByRole('heading', { name: 'What this fixture checks' })
    .evaluate((heading) => window.scrollTo(0, heading.offsetTop - 90));
  await expect(
    contents.getByRole('link', { name: 'What this fixture checks' }),
  ).toHaveAttribute('aria-current', 'location');
  ...
  await page.screenshot({
    path: 'visual-review/1536-writings-index.png',
    fullPage: false,
  });
});

test('mobile Contents control appears only after the TOC, preserves state, and respects reduced motion', async ({
  page,
}) => {
  ...
  await page.screenshot({
    path: 'visual-review/412-writings-contents-visible.png',
    fullPage: false,
  });
  ...
  await expect(
    contents.getByRole('link', { name: 'Equivalent examples' }),
  ).toHaveAttribute('aria-current', 'location');
  ...
  expect(
    buttonBox &&
      bottomNavBox &&
      buttonBox.y + buttonBox.height < bottomNavBox.y,
  ).toBeTruthy();
  await page.screenshot({
    path: 'visual-review/412-writings-deep-contents-control.png',
    fullPage: false,
  });
  ...
  await page.evaluate(() => {
    ...
    Element.prototype.scrollIntoView = function (
      ...
      (
        window as typeof window & { __contentsScrollBehavior?: ScrollBehavior }
      ).__contentsScrollBehavior =
      ...
      () =>
        (
          window as typeof window & {
            __contentsScrollBehavior?: ScrollBehavior;
          }
        ).__contentsScrollBehavior,
    ),
  ).toBe('auto');
  await page.screenshot({
    path: 'visual-review/412-writings-after-contents-return.png',
    fullPage: false,
  });

  await page
    .getByRole('heading', { name: 'Equivalent examples' })
    .scrollIntoViewIfNeeded();
  await expect(
    page.getByRole('tab', { name: 'C#', selected: true }),
  ).toBeVisible();
  await page.screenshot({
    path: 'visual-review/412-writings-code-tabs.png',
    fullPage: false,
  });
  ...
  await expect(returnButton).toHaveCount(0);
  await page.screenshot({
    path: 'visual-review/412-writings-index.png',
    fullPage: false,
  });
});

test('standalone plaintext uses the article width, wraps visually, and copies exactly', async ({
  page,
  context,
}) => {
  await context.grantPermissions(['clipboard-read', 'clipboard-write'], {
  ...
  const articleSource = await readFile(
    new URL(
      '../../src/content/writings/published/when-the-workaround-becomes-the-architecture.md',
      import.meta.url,
    ),
    'utf8',
  );
  ...
  for (const viewport of [
    ...
    await page.setViewportSize(viewport);
    await page.goto(
      './writings/when-the-workaround-becomes-the-architecture#make-the-decision-explicit',
    );
    await page.evaluate(() => {
    ...
    const presentation = await plaintext.evaluate((pre) => {
      ...
      if (!figure || !code || !figure.parentElement)
        throw new Error('Plaintext code structure is missing.');
    ...
    const copyButton = plaintext
      .locator('..')
      .getByRole('button', { name: 'Copy Plain text code' });
...
test('programming blocks and code tabs keep non-wrapping source-code behavior', async ({
  page,
}) => {
  for (const viewport of [
    { width: 375, height: 667 },
...
for (const viewport of [
  ...
  test(`writing remains responsive at ${viewport.width}x${viewport.height}`, async ({
    page,
  }) => {
    await page.setViewportSize(viewport);
    await page.goto(`${writingPath}#equivalent-examples`);
    await expect(
      page.getByRole('heading', { name: 'Equivalent examples' }),
    ).toBeInViewport();
    await expect(
      page
        .getByRole('navigation', { name: 'Contents' })
        .getByRole('link', { name: 'Equivalent examples' }),
    ).toHaveAttribute('aria-current', 'location');
    expect(
      await page.evaluate(
        () =>
    ...
    if (viewport.width < 768) {
      await page
        .getByRole('heading', { name: 'Portable Markdown elements' })
        .scrollIntoViewIfNeeded();
      ...
      const navBox = await page
        ...
        .boundingBox();
      expect(
        controlBox && navBox && controlBox.y + controlBox.height < navBox.y,
      ).toBeTruthy();
    } else {
      await expect(
        page.getByRole('button', { name: /Return to this writing's Contents/ }),
